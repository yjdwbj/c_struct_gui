 ...
 */
DIAVAR void dia_font_init(PangoContext* pcontext);
/* Start using a new context (for AA rendering) */
DIAVAR void dia_font_push_context(PangoContext *pcontext);
/* Go back to using the old context */
DIAVAR void dia_font_pop_context(void);
/* Retrieve the current context (used for the font widget) */
DIAVAR PangoContext *dia_font_get_context(void);

    /* Get a font matching family,style,height. MUST be freed with
       dia_font_unref(). */
DIAVAR DiaFont* dia_font_new(const char *family, DiaFontStyle style,
                      real height);

    /* Get a font matching style. This is the preferred method to
     * create default fonts within objects. */
DIAVAR DiaFont* dia_font_new_from_style(DiaFontStyle style, real height);

    /* Get a font from a legacy font name */
DIAVAR DiaFont* dia_font_new_from_legacy_name(const char *name);

    /* Get a simple font name from a font.
       Name will be valid for the duration of the DiaFont* lifetime. */
DIAVAR G_CONST_RETURN char* dia_font_get_legacy_name(const DiaFont* font);

    /* Same attributes */
DIAVAR DiaFont *dia_font_copy(const DiaFont* font);

extern __declspec(dllexport) DiaFont* dia_font_ref(DiaFont* font);
extern __declspec(dllexport)  void dia_font_unref(DiaFont* font);

    /* Retrieves the style of the font */
DIAVAR DiaFontStyle dia_font_get_style(const DiaFont* font);

    /* Retrieves the family of the font. Caller must NOT free. */
DIAVAR G_CONST_RETURN char* dia_font_get_family(const DiaFont* font);
/* Acessor for the PangoFontDescription */
DIAVAR G_CONST_RETURN PangoFontDescription *dia_font_get_description (const DiaFont* font);

    /* Retrieves the height of the font */
DIAVAR real dia_font_get_height(const DiaFont* font);
    /* Change the height inside a font record. */
DIAVAR void dia_font_set_height(DiaFont* font, real height);
    /* Delivers the size of the font */
DIAVAR real dia_font_get_size(const DiaFont* font);

    /* Changes the slant of an existing font */
DIAVAR void dia_font_set_slant(DiaFont* font, DiaFontSlant slant);
    /* Changes the weight of an existing font */
DIAVAR void dia_font_set_weight(DiaFont* font, DiaFontWeight weight);
...
void dia_font_set_family(DiaFont* font, DiaFontFamily family);
    /* Changes the family of an existing font to any family, but the name could
       be system-dependent. */
DIAVAR void dia_font_set_any_family(DiaFont* font, const char* family);

    /* FIXME: what do we do with this, actually ?
       Name lives for as long as the DiaFont lives. */
DIAVAR G_CONST_RETURN char *dia_font_get_psfontname(const DiaFont *font);

    /* returns a static string suitable for SVG */
DIAVAR G_CONST_RETURN char *dia_font_get_weight_string(const DiaFont* font);

    /* returns a static string suitable for SVG */
DIAVAR G_CONST_RETURN char *dia_font_get_slant_string(const DiaFont* font);

    /* uses an SVG style string */
DIAVAR void dia_font_set_weight_from_string(DiaFont* font, const char* weight);

    /* uses an SVG style string */
DIAVAR void dia_font_set_slant_from_string(DiaFont* font, const char* slant);
...
extern __declspec(dllexport) real dia_font_string_width(const char* string, DiaFont* font,
                          real height);
    /* Get the ascent of this string in cm (a positive number). */
DIAVAR real dia_font_ascent(const char* string, DiaFont* font, real height);
    /* Get the descent of the font in cm (a positive number) */
DIAVAR real dia_font_descent(const char* string, DiaFont* font, real height);

    /* prepares a layout of the text, in font 'font'. */
DIAVAR PangoLayout* dia_font_build_layout(const char* string, DiaFont* font,
                                   real height);
/* prepares a layout of the text, in font 'font' scaled by matrix. */
DIAVAR PangoLayout* dia_font_build_layout_with_matrix(const char* string,
					       DiaFont* font,
					       real height,
					       PangoMatrix *matrix);

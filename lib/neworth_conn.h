...
DIAVAR void neworthconn_update_data(NewOrthConn *orth);
DIAVAR void neworthconn_update_boundingbox(NewOrthConn *orth);
void neworthconn_simple_draw(NewOrthConn *orth, DiaRenderer *renderer,
			     real width);
DIAVAR void neworthconn_init(NewOrthConn *orth, Point *startpoint);
DIAVAR void neworthconn_destroy(NewOrthConn *orth);
DIAVAR void neworthconn_copy(NewOrthConn *from, NewOrthConn *to);
DIAVAR void neworthconn_save(NewOrthConn *orth, ObjectNode obj_node);
DIAVAR void neworthconn_load(NewOrthConn *orth, ObjectNode obj_node);  /* NOTE: Does object_init() */
DIAVAR ObjectChange* neworthconn_move_handle(NewOrthConn *orth, Handle *id,
				      Point *to, ConnectionPoint *cp,
				      HandleMoveReason reason,
				      ModifierKeys modifiers);
DIAVAR ObjectChange* neworthconn_move(NewOrthConn *orth, Point *to);
DIAVAR real neworthconn_distance_from(NewOrthConn *orth, Point *point,
			       real line_width);
Handle* neworthconn_get_middle_handle(NewOrthConn *orth);

DIAVAR int neworthconn_can_delete_segment(NewOrthConn *orth, Point *clickedpoint);
DIAVAR int neworthconn_can_add_segment(NewOrthConn *orth, Point *clickedpoint);
DIAVAR ObjectChange *neworthconn_delete_segment(NewOrthConn *orth, Point *clickedpoint);
DIAVAR ObjectChange *neworthconn_add_segment(NewOrthConn *orth, Point *clickedpoint);

...
dia_register_plugins(void)
{
 ...
 //library_path = g_get_current_dir();
 library_path = NULL;
...
//    for (i = 0; paths[i] != NULL; i++) {
      const int len = strlen(library_path);
      char ext[] = "\\dia\0";
      gchar* newdir = g_new(char,len+strlen(ext)+1);
      strncpy(newdir,library_path,len);
      strncpy(&newdir[len],ext,strlen(ext));
      dia_register_plugins_in_dir(newdir);
      g_free(newdir);
      g_free(library_path);
//    }
//    g_strfreev(paths);

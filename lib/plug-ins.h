...
DIAVAR gboolean dia_plugin_info_init(PluginInfo *info, gchar *name,
			      gchar *description,
			      PluginCanUnloadFunc can_unload_func,
			      PluginUnloadFunc unload_func);

DIAVAR gchar *dia_plugin_check_version(gint version);


/* functiosn for use by dia ... */
DIAVAR const gchar *dia_plugin_get_filename    (PluginInfo *info);
DIAVAR const gchar *dia_plugin_get_name        (PluginInfo *info);
DIAVAR const gchar *dia_plugin_get_description (PluginInfo *info);
DIAVAR const gpointer *dia_plugin_get_symbol   (PluginInfo *info, const gchar *name);

DIAVAR gboolean dia_plugin_can_unload       (PluginInfo *info);
DIAVAR gboolean dia_plugin_is_loaded        (PluginInfo *info);
DIAVAR gboolean dia_plugin_get_inhibit_load (PluginInfo *info);
DIAVAR void     dia_plugin_set_inhibit_load (PluginInfo *info, gboolean inhibit_load);

DIAVAR void dia_plugin_load   (PluginInfo *info);
DIAVAR void dia_plugin_unload (PluginInfo *info);

DIAVAR void dia_register_plugin         (const gchar *filename);
DIAVAR void dia_register_plugins_in_dir (const gchar *directory);
DIAVAR  void dia_register_plugins        (void);
DIAVAR void dia_register_builtin_plugin (PluginInitFunc init_func);

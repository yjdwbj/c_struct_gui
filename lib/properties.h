...
DIAVAR void prop_desc_list_calculate_quarks(PropDescription *plist);
/* plist must have all quarks calculated in advance */
DIAVAR const PropDescription *prop_desc_list_find_prop(const PropDescription *plist,
                                                const gchar *name);
/* finds the real handler in case there are several levels of indirection */
DIAVAR  PropEventHandler prop_desc_find_real_handler(const PropDescription *pdesc);
/* free a handler indirection list */
DIAVAR  void prop_desc_free_handler_chain(PropDescription *pdesc);
/* free a handler indirection list in a list of descriptors */
DIAVAR  void prop_desc_list_free_handler_chain(PropDescription *pdesc);
/* insert an event handler */
DIAVAR  void prop_desc_insert_handler(PropDescription *pdesc,
                              PropEventHandler handler);

/* operations on lists of property description lists */
DIAVAR PropDescription *prop_desc_lists_union(GList *plists);
DIAVAR PropDescription *prop_desc_lists_intersection(GList *plists);
...
DIAVAR void       prop_list_free(GPtrArray *plist);


/* copies the whole property structure, including the data. */
DIAVAR GPtrArray *prop_list_copy(GPtrArray *plist);
/* copies the whole property structure, excluding the data. */
DIAVAR GPtrArray *prop_list_copy_empty(GPtrArray *plist);
/* Appends copies of the properties in the second list to the first. */
DIAVAR void prop_list_add_list (GPtrArray *props, const GPtrArray *ptoadd);

DIAVAR GPtrArray *prop_list_from_descs(const PropDescription *plist,
                                PropDescToPropPredicate pred);

DIAVAR GPtrArray *prop_list_from_single(Property *prop);

/* Some predicates: */
DIAVAR gboolean pdtpp_true(const PropDescription *pdesc); /* always true */
DIAVAR gboolean pdtpp_is_visible(const PropDescription *pdesc);
...
/* Swallows the property into a single property list. Can be given NULL.
   ...
   list. */
DIAVAR GPtrArray *prop_list_of_single(Property *prop);
...
DIAVAR Property *make_new_prop(const char *name, PropertyType type, guint flags);
...
DIAVAR const PropDescription *object_get_prop_descriptions(const DiaObject *obj);
const PropDescription *object_list_get_prop_descriptions(GList *objects,
							 PropMergeOption option);

DIAVAR gboolean object_get_props_from_offsets(DiaObject *obj, PropOffset *offsets,
				       GPtrArray *props);
DIAVAR gboolean object_set_props_from_offsets(DiaObject *obj, PropOffset *offsets,
				       GPtrArray *props);

/* apply some properties and return a corresponding object change */
DIAVAR ObjectChange *object_apply_props(DiaObject *obj, GPtrArray *props);
 ...
 */
DIAVAR WIDGET *object_create_props_dialog     (DiaObject *obj, gboolean is_default);
DIAVAR WIDGET *object_list_create_props_dialog(GList *obj, gboolean is_default);
DIAVAR ObjectChange *object_apply_props_from_dialog (DiaObject *obj, WIDGET *dialog);
...
/* create a property from the object's property descriptors. To be freed with
   ...
   Serve cold. */
DIAVAR Property *object_prop_by_name(DiaObject *obj, const char *name);
DIAVAR Property *object_prop_by_name_type(DiaObject *obj, const char *name, const char *type);

/* standard way to load/save properties of an object */
DIAVAR void          object_load_props(DiaObject *obj, ObjectNode obj_node);
DIAVAR void          object_save_props(DiaObject *obj, ObjectNode obj_node);

/* standard way to copy the properties of an object into another (of the
   same type) */
DIAVAR void          object_copy_props(DiaObject *dest, const DiaObject *src,
                                gboolean is_default);

/* Return a reference to objects property with 'name' or NULL */
DIAVAR Property     *object_get_prop_by_name (DiaObject *obj, const char* name);

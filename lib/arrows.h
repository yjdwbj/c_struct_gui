...
struct _Arrow {
 ...
 extern __declspec(dllexport) void arrow_draw(DiaRenderer *renderer, ArrowType type,
		Point *to, Point *from,
...
extern __declspec(dllexport) void arrow_bbox (const Arrow *arrow, real line_width, const Point *to, const Point *from,
                 Rectangle *rect);
...
DIAVAR void save_arrow(ObjectNode obj_node, Arrow *arrow, gchar *type_attribute,
		gchar *length_attribute, gchar *width_attribute);
DIAVAR void load_arrow(ObjectNode obj_node, Arrow *arrow, gchar *type_attribute,
		gchar *length_attribute, gchar *width_attribute);

/** Returns the ArrowType for a given name of an arrow, or 0 if not found. */
DIAVAR ArrowType arrow_type_from_name(const gchar *name);
...
DIAVAR const gchar *arrow_get_name_from_type(ArrowType type);
DIAVAR GList *get_arrow_names(void);

...
DIAVAR  void point_convex(Point *dst, const Point *src1, const Point *src2, real alpha);

DIAVAR void rectangle_union(Rectangle *r1, const Rectangle *r2);
DIAVAR  void int_rectangle_union(IntRectangle *r1, const IntRectangle *r2);
DIAVAR  void rectangle_intersection(Rectangle *r1, const Rectangle *r2);
DIAVAR  int rectangle_intersects(const Rectangle *r1, const Rectangle *r2);
int point_in_rectangle(const Rectangle* r, const Point *p);
DIAVAR  int rectangle_in_rectangle(const Rectangle* outer, const Rectangle *inner);
DIAVAR  void rectangle_add_point(Rectangle *r, const Point *p);
...
DIAVAR real distance_rectangle_point(const Rectangle *rect, const Point *point);
DIAVAR real distance_line_point(const Point *line_start, const Point *line_end,
			 real line_width, const Point *point);

DIAVAR real distance_polygon_point(const Point *poly, guint npoints,
			    real line_width, const Point *point);

/* bezier distance calculations */
DIAVAR real distance_bez_seg_point(const Point *b1, const Point *b2,
                            const Point *b3, const Point *b4,
			    real line_width, const Point *point);
DIAVAR real distance_bez_line_point(const BezPoint *b, guint npoints,
			     real line_width, const Point *point);
DIAVAR real distance_bez_shape_point(const BezPoint *b, guint npoints,
			      real line_width, const Point *point);

DIAVAR real distance_ellipse_point(const Point *centre, real width, real height,
			    real line_width, const Point *point);

typedef real  Vector[3];
typedef Vector  Matrix[3];

DIAVAR void          transform_point             (Matrix, Point *, Point *);
void          mult_matrix                 (Matrix, Matrix);
DIAVAR void          identity_matrix             (Matrix);
void          translate_matrix            (Matrix, real, real);
void          scale_matrix                (Matrix, real, real);
DIAVAR void          rotate_matrix               (Matrix, real);
...
DIAVAR int  three_point_circle(const Point *p1, const Point *p2, const Point *p3,
                        Point* center, real* radius);
DIAVAR real point_cross(Point *p1, Point *p2);
DIAVAR Point calculate_object_edge(Point *objmid, Point *end, DiaObject *obj);

...
DIAVAR void filter_register_export(DiaExportFilter *efilter);
/* except - maybe - after unregistering it */
DIAVAR void filter_unregister_export(DiaExportFilter *efilter);

/* returns a sorted list of the export filters. */
DIAVAR GList *filter_get_export_filters(void);

/* creates a nice label for the export filter (must be g_free'd) */
DIAVAR gchar *filter_get_export_filter_label(DiaExportFilter *efilter);

/* guess the filter for a given filename. */
DIAVAR DiaExportFilter *filter_guess_export_filter(const gchar *filename);
/* Get the filter for the unique filename. */
DIAVAR DiaExportFilter *filter_get_by_name(const gchar *name);
/* Get the list of unique names for the given extension */
DIAVAR GList *filter_get_unique_export_names(const char *extension);
/* Set the favorit 'guess' */
DIAVAR void filter_set_favored_export(const char *extension, const char *name);

DIAVAR void filter_register_import(DiaImportFilter *ifilter);
DIAVAR void filter_unregister_import(DiaImportFilter *ifilter);
DIAVAR GList *filter_get_import_filters(void);
DIAVAR gchar *filter_get_import_filter_label(DiaImportFilter *ifilter);
DIAVAR DiaImportFilter *filter_guess_import_filter(const gchar *filename);

DIAVAR void filter_register_callback(DiaCallbackFilter *cbfilter);
/* returns all registered callbacks */
DIAVAR GList *filter_get_callbacks(void);

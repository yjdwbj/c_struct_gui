...
DIAVAR void connection_update_handles(Connection *conn);
DIAVAR void connection_update_boundingbox(Connection *conn);
DIAVAR void connection_init(Connection *conn,
		     int num_handles, int num_connections);
DIAVAR void connection_destroy(Connection *conn);
DIAVAR void connection_copy(Connection *from, Connection *to);
DIAVAR void connection_save(Connection *conn, ObjectNode obj_node);
DIAVAR void connection_load(Connection *conn, ObjectNode obj_node);
DIAVAR ObjectChange* connection_move_handle(Connection *conn, HandleId id,
				     Point *to, ConnectionPoint* cp,
				     HandleMoveReason reason,
				     ModifierKeys modifiers);

DIAVAR void connection_adjust_for_autogap(Connection *conn);

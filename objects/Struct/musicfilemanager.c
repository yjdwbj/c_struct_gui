...
//        GtkTreeModel *model = gtk_tree_view_get_model(smd->smfm->wid_treeview);
        ...
        {
            int pos = g_strtod(sss->value.vnumber,NULL) - smd->smfm->offset;
            if(pos > 0)
            {
...
void *factory_read_mfile_item_from_xml(SaveStruct *sss,FactoryStructItem *fst,ObjectNode attr_node)
{

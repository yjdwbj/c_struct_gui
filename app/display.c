...
void factory_add_item_to_idlist(GtkWidget *self)
{
    ...
    int w = 0;
    int h = 0;
    gdk_drawable_get_size(self->window,&w,&h);
    GtkWidget *twidget = gtk_widget_get_toplevel(self);

    gtk_widget_set_size_request (twidget,w,h);
...
void factory_add_item_to_music_manager(GtkWidget *self)
{
    ...
    int w = 0;
    int h = 0;
    gdk_drawable_get_size(self,&w,&h);
...
GtkWidget *factory_get_new_item(int id)
{
    ...
    GList *p = active_display->diagram->data->active_layer->defnames;
    for(; p ; p = p->next)
    {
...
void factory_idlist_dialog(gchar *title,GtkAction *action)
{
    ...
    gtk_box_pack_start(GTK_BOX(vbox),factory_new_add_button(factory_add_item_to_idlist,title),FALSE,FALSE,0);
...
void factory_music_filemanager_dialog(gchar *title,GtkAction *action)
{
    ...
    gtk_container_add(GTK_CONTAINER(dialog_vbox),sdialog);
    gtk_window_set_resizable (GTK_WINDOW (newdialog),TRUE);
    gtk_widget_set_size_request (GTK_WINDOW (newdialog),300,500);
    ...
    gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(wid_idlist),vbox);
    gtk_box_pack_start(GTK_BOX(vbox),factory_get_new_item_head(),FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(vbox),factory_new_add_button(factory_add_item_to_music_manager,title),FALSE,FALSE,0);

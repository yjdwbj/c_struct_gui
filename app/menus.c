...
void factory_callback_muisc_convert(GtkWidget *btn,gpointer user_data)
{
    ...
    g_return_if_fail(factory_test_file_exist(cmd));
...
void factory_callback_system_data(GtkWidget *btn,gpointer user_data)
{
    ...
    if(factoryContainer->sys_info)
    {
        ...
        fstype->sysinfo_callback(user_data); /* 系统设置 */
    }
}
...
create_integrated_ui_toolbar (void)
{
    ...
    g_signal_connect(sbtn,"clicked",G_CALLBACK(factory_callback_system_data),toolbar);
    ...
    dbtn = gtk_button_new_with_label(factory_utf8("模版编辑"));
    integrated_ui_toolbar_add_custom_item(toolbar,dbtn);
    ...
    dbtn = gtk_button_new_with_label(factory_utf8("模版加载"));
    integrated_ui_toolbar_add_custom_item(toolbar,dbtn);
